  ...
  switch (authType) {
    ...
    case 'oauth 1.0':
      strategy = new AuthStrategy(_.defaults({
        ...
        function(req, token, tokenSecret, profile, done) {
          console.log("GOT TO OAUTH CALLBACK");
          if (link) {
            console.log(req);
            ...
            if (req.accessToken && req.accessToken.userId) {
              self.userCredentialModel.link(
                String(req.accessToken.userId), provider, authScheme, profile,
    ...
    case 'openid':
      strategy = new AuthStrategy(_.defaults({
        ...
        function(req, identifier, profile, done) {
          console.log("GOT TO OPENID CALLBACK");
          if (link) {
            if (req.accessToken && req.accessToken.userId) {
              self.userCredentialModel.link(
                req.accessToken.userId, provider, authScheme, profile,
    ...
    case 'openid connect':
      strategy = new AuthStrategy(_.defaults({
        ...
        function(req, iss, sub, profile, jwtClaims, accessToken, refreshToken,
          params, done) {
        console.log("GOT TO OPENID CONNECT AUTH");
    ...
    case 'saml':
      strategy = new AuthStrategy(_.defaults({
        ...
        console.log("GOT TO SAML AUTH");
          if (link) {
            if (req.accessToken && req.accessToken.userId) {
              self.userCredentialModel.link(req.accessToken.userId, name, authScheme,
    ...
    default:
      strategy = new AuthStrategy(_.defaults({
        ...
        function(req, accessToken, refreshToken, params2, profile, done) {
          ...
          if (provider === "apple") {
            ...
            if (req.appleProfile) {
              if (req.appleProfile.name) {
                newProfile.name = `${req.appleProfile.name.firstName} ${req.appleProfile.name.lastName}`;
              }
            }
  ...
  if (link) {
    self.app[callbackHTTPMethod](callbackPath, passport.authorize(name, _.defaults({
      ...
      function(req, res, next) {
        ...
        if (options.failureFlashV2) {
          if (_.isFunction(req.pushMessage)) {
            req.pushMessage({
              type: err.flashType || 'error',
              title: err.flashTitle || 'Error Occurred',
              description: err.flashMessage || (err.flashMessage || (err.message || "An Unknown Error Occurred"))
            });
          }
        }
